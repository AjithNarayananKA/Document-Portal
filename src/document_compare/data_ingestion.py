...
class DocumentIngestion:
    ...
    def save_uploaded_files(self,reference_file, actual_file):
        ...
        try:
            ...
            if not reference_file.lower().endswith(".pdf") or not actual_file.lower().endswith(".pdf"):
    ...
    def read_pdf(self, pdf_path: Path)-> str:
        """Reads a PDF files and extracts text from each page."""
        try:
            with fitz.open(pdf_path,"rb") as doc:
                if doc.is_encrypted:
                    raise ValueError(f"PDF is encrypted: {pdf_path.name}")
                text_chunks = []
                for page_num in range(pdf_path.page_count):
